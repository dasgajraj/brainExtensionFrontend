...
const Onboarding1: React.FC<Onboarding1Props> = ({ onNext, onSkip, theme = 'dark' }) => {
  ...
  useEffect(() => {
    ...
    particles.forEach((particle, index) => {
      ...
      setTimeout(() => {
        particle.scale.value = withSpring(1, { damping: 8 });
        particle.x.value = withRepeat(
          withSequence(
            withTiming(Math.cos(angle) * radius, { duration: 3000 }),
            withTiming(Math.cos(angle + Math.PI) * radius, { duration: 3000 })
          ),
          -1,
          true
        );
        particle.y.value = withRepeat(
          withSequence(
            withTiming(Math.sin(angle) * radius, { duration: 3000 }),
            withTiming(Math.sin(angle + Math.PI) * radius, { duration: 3000 })
          ),
          -1,
          true
        );
  ...
  const blob1Style = useAnimatedStyle(() => {
    const scale = interpolate(blob1.value, [0, 1], [1, 1.3]);
    const translateX = interpolate(blob1.value, [0, 1], [0, 30]);
    return {
      transform: [{ scale }, { translateX }],
      opacity: interpolate(blob1.value, [0, 1], [0.6, 0.9]),
    };
  });
  
  const blob2Style = useAnimatedStyle(() => {
    const scale = interpolate(blob2.value, [0, 1], [1, 1.2]);
    const translateY = interpolate(blob2.value, [0, 1], [0, -40]);
    return {
      transform: [{ scale }, { translateY }],
      opacity: interpolate(blob2.value, [0, 1], [0.5, 0.8]),
    };
  });
  
  const blob3Style = useAnimatedStyle(() => {
    const scale = interpolate(blob3.value, [0, 1], [1, 1.25]);
    const rotate = interpolate(blob3.value, [0, 1], [0, 180]);
    return {
      transform: [{ scale }, { rotate: `${rotate}deg` }],
      opacity: interpolate(blob3.value, [0, 1], [0.4, 0.7]),
    };
  });
        ...
        {particles.map((particle, index) => {
          ...
          return (
            <Animated.View key={index} style={[styles.particle, particleStyle]}>
              <Text style={styles.particleIcon}>{icons[index]}</Text>
            </Animated.View>
          );
        })}
      </View>
